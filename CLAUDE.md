...
| 2026-05-18 | `18-MID-perfil-v2-lgpd` (Codex) | ✅ Perfil V2 compliance: conta, privacidade, suporte, sair e exclusão de conta em ≤2 taps. Delete via Edge Function `delete-user-account`, typed confirm `EXCLUIR`, `AuthButton` variant `destructive`, URLs externas registradas como pendência pre-ship. Delete real não executado no usuário de teste. | (PR pendente) |
